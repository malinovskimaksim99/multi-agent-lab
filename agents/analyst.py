...
class AnalystAgent(BaseAgent):
    ...
    def run(self, task: str, memory: Memory, context: Optional[Context] = None) -> AgentResult:
        ...
        # --- 5) Спеціальний план розвитку multi-agent-lab ---
        if _is_planning(t) and ("multi-agent-lab" in t or "multi agent lab" in t):
            output = (
                "## План розвитку multi-agent-lab\n\n"
                "### Ціль\n"
                "Побудувати гнучкий мультиагентний стенд, який можна розширювати новими агентами "
                "і використовувати для навчання та експериментів.\n\n"
                "### Кроки\n"
                "1. **Кодова база і стабільність**: завершити інтеграцію БД, конфігів агентів і "
                "meta-агента, прибрати тимчасові хардкоди.\n"
                "2. **Роутинг і конфіги**: налаштувати router так, щоб він спирався на task_type "
                "та agent_configs, а не лише на прості правила.\n"
                "3. **Спеціалізовані агенти**: поступово додавати нових агентів (наприклад, під документацію, "
                "аналітику коду, майбутній SDR тощо) за єдиним шаблоном.\n"
                "4. **Інтерфейс взаємодії**: відшліфувати CLI/чат-режим так, щоб основний \"фронт-агент\" "
                "міг приймати задачі і делегувати їх іншим.\n"
                "5. **Навчання і аналіз**: розширити тренувальні сценарії, використовуючи датасет, критика "
                "та trainer/meta для покращення поведінки агентів.\n\n"
                "### Перевірка результату\n"
                "- Запустити кілька типових сценаріїв (планування, пояснення, документація, аналіз БД) "
                "і переконатися, що задачі потрапляють до правильних агентів і дають стабільні відповіді.\n"
                "- Перевірити, що meta-агент може побудувати схожий план самостійно на основі БД.\n\n"
                "### Приклади команд\n"
                "- `python app.py --task \"Склади план розвитку multi-agent-lab.\" --auto`\n"
                "- `python app.py --task \"Зроби аналіз 50 останніх запусків у БД.\" --auto`\n"
            )
            return AgentResult(
                agent=self.name,
                output=output,
                meta={"mode": "project_plan"},
            )
